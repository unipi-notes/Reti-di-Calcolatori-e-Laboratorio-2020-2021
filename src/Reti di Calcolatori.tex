...
\begin{definition}
	...
	\fbox{\centering{$L = r_{propagazione} + r_{trasmissione} + r_{accodamento} + r_{elaborazione}$}}
\end{definition}
\newpage
...
\newpage
\begin{figure}[!h]
	\includegraphics[scale=0.5]{Immagini/Connection_oriented.png}
	\centering
...
\textbf{\underline{N.B.}} \textbf{Solo} i processi che inviano messaggi di dimensione \textbf{inferiore a 65507}, i.e. $65535 - 8$ byte di intestazione UDP $ - 20$ di intestazione, IP di \textbf{possono usare il protcollo UDP}.
\begin{figure}[!h]
...
\end{figure}\newline
Il \textbf{segmento consiste} di un'\textbf{intestazione} di dimensione compresa tra 20 byte, in assenza di opzioni, e 60 byte altrimenti, seguita dai \textbf{dati} proveniente dal processo applicativo.
\begin{itemize}
...
\end{flushleft}
Il \textbf{protocollo TCP} prevede due finestre, una di \textbf{ricezione} e una di \textbf{trasmissione}, per ogni direzione di comunicazione, in totale sono quindi presenti \textbf{4 finestre}. Ricordiamo infatti che la connessione TCP è \textbf{bidirezionale}. La \textbf{dimensione della finestra di trasmissione} è \textit{\textbf{inizialmente}} determinata all'apertura delle connessione. Si \textbf{apre} quando la dimensione della finestra di ricezione segnalata dal TCP destinatario lo consente e si \textbf{chiude} quando viene confermata la ricezione dei byte inviati. \textbf{Apertura, chiusura e ridimensionamento sono controllate dal TCP destinatario}.
\begin{figure}[!h]
...
\end{figure}\newline
\centerline{\fbox{\textbf{rwindow = rwindowInit - (LastByteReceived - LastByteRead)}}}\newline\newline
