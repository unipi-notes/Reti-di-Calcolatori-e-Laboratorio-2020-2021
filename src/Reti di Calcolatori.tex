...
\begin{enumerate}
	...
	\item Un client C chiede la pagina web
	      ...
	      campo DATA, in ciascuno dei segmenti che C e B si scambianoper aprire la connessione nell’ipotesi che l’ack finale dell’apertura della connessione sia inviato in piggybacking
	      assieme alla GET. Si supponga che in B ed in C receivewnd sia molto grande, che
	...
	\item Si descriva il meccanismo di controllo di flusso in
	      ...
	      \end{flushleft}
	      Si intende con controllo di flusso la capacità del mittente di evitare la possibilità di saturare il buffer del ricevitore. Infatti, a livello TCP ogni host imposta un buffer di invio e uno di ricezione. Il processo applicativo
		...
		if (!finito) {
			...
			if (OK(segm)) { 
				...
				if (nuovo(segm.seqnum)) { 
					...
					risposta.ACK = true; 
					risposta.ACKnum = calcolacknum(segm);2
					risposta.rwnd = rwnd;
				}
	...
	\end{lstlisting}
	      \end{flushleft}
	\item Discutere l'affermazione “poiché FTP e HTTP sono protocolli adatti a trasferire file, possono essere usati indifferentemente”.
	      \begin{flushleft}
